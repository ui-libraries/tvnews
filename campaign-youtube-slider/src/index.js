...
let isKeyReleased = false;  // New variable to keep track of key release

// Function to handle the logic when the key/slider is released
function handleKeyOrSliderRelease() {
    if (!isKeyReleased) {
        let videoNum = parseInt(localStorage.getItem('videoIndex')) - 1;
        let currentVideoID = videoList[videoNum];
        player.getCurrentTime().then(value => {
            let newValue = _.toInteger(document.getElementById('slider').value);
            let time = {
                "Seconds": Math.trunc(value),
...
// Event listener for 'keydown' to move the slider
document.addEventListener('keydown', function(event) {
    let slider = document.getElementById('slider');
    let sliderValue = _.toNumber(slider.value);
    if (event.key === 'a') {
        sliderValue = Math.max(sliderValue - 1, slider.min);  // decrease value but not less than min
    } else if (event.key === 'd') {
        sliderValue = Math.min(sliderValue + 1, slider.max);  // increase value but not more than max
    }
...
// Event listener for 'keyup' to capture the release of the key
document.addEventListener('keyup', function(event) {
    if (event.key === 'a' || event.key === 'd') {
        handleKeyOrSliderRelease();
    }
});

// Event listener for slider 'change' event
slider.addEventListener('change', function() {
    handleKeyOrSliderRelease();
});
